...
while True:
    ...
    if team_state == "TEAM STATIONARY":
        ...
        # STATIONARYが30フレーム以上続いた後，
        # MOVINGになった瞬間を保存
        if prev_team_state == "TEAM STATIONARY" and stationary_frames >= 30:

            filename = f"snap_{snap_count}.jpg"
            cv2.imwrite(filename, frame)

            print(f"{filename} saved (Frame {frame_number})")
